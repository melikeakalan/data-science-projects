...
df = pd.read_csv("baseball_salary/hitters.csv")
df.head()
df.shape
...
# Grafik Teknikle Aykırı Değer
sns.boxplot(x=df["CAtBat"])
plt.show()

sns.boxplot(x=df["CHits"])
plt.show()

sns.boxplot(x=df["CHmRun"])
plt.show()
...
# salary değişkeni, sağa çarpık dağılımda
# bunu normal dağılıma dönüştürmek için: Logaritmik, karekök, MinMax dönüşümleri uygulanabilir
# Burada zaten NaN ler baskılanacak ve outlier olanlar veri setinden çıkarılacağı için normalleştirilmiş olur
...
# CHmRun ve CHits değişkenlerinde aykırı gözlem var

# 1350 den sonraki gözlemleri veri setinden çıkartıyorum.
# Eksik gözlemleri çıkartmıyorum.
print(df.shape)
df = df[(df['Salary'] < 1350) | (df['Salary'].isnull())]
print(df.shape)
# 14 tane aykırı gözlem veri setinden çıkartılmış oldu

# CAtBat değişkeni için deniyorum, df'ye kaydetmiyrum
print(df.shape)
new_df = df[(df['CAtBat'] < 8000) | (df['CAtBat'].isnull())]
print(new_df.shape)

# aykırı değerler, değişkenşn 9 tane aykırı gözlemi var
df[df["CAtBat"] > 8000]

# salary, normal dağılıma yaklaşmış
sns.distplot(df.Salary)
plt.show()
